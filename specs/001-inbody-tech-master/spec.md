...
사용자가 웹 브라우저를 통해 InBody 기술 지원 챗봇에 접속하면, Streamlit 기반 채팅 인터페이스에서 대화를 진행할 수 있다. 기종 선택, 이미지 업로드, 스트리밍 응답 등의 기능을 GUI로 제공하며, AWS EC2에 배포되어 평일 09:00~19:00에 자동으로 운영된다.

**우선순위 근거**: 모든 백엔드 에이전트가 구현된 후에 프론트엔드를 연결해야 완전한 사용자 경험을 제공할 수 있다. 배포는 데모 시연을 위한 최종 단계이다.

**독립 테스트**: 브라우저에서 Streamlit UI에 접속하여 기종을 선택하고, 기술 지원 질문을 입력했을 때 스트리밍으로 응답이 표시되는지 확인한다.
...
3. **Given** 사용자가 기기 사진을 업로드한 상태, **When** 시스템이 이미지를 분석하면, **Then** 식별된 기종이 사이드바에 자동 반영되고 확인 메시지가 표시된다.
4. **Given** EC2 인스턴스가 스케줄에 따라 09:00 KST에 시작된 상태, **When** 사용자가 브라우저로 접속하면, **Then** Streamlit UI와 FastAPI 백엔드가 정상 동작한다.
