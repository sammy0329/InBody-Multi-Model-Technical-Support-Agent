...
사용자가 대화를 시작하면, 시스템은 사용자의 InBody 기종(270S, 580, 770S, 970S)을 식별하고, 해당 기종 전용 지원 흐름으로 자동 분기한다. 기종 식별은 사용자가 직접 기종을 선택하거나 텍스트로 모델명을 입력하는 방식으로 이루어진다.

**우선순위 근거**: 기종 식별은 모든 후속 에이전트(설치, 연동, 트러블슈팅, 임상 방어)의 전제 조건이다. 기종이 식별되지 않으면 어떤 기술 지원도 제공할 수 없으므로 최우선 순위이다.

**독립 테스트**: 기종 식별 단독으로 테스트 가능 — 사용자가 기종을 선택하거나 모델명을 입력했을 때, 시스템이 올바른 기종을 식별하고 해당 기종 전용 환영 메시지를 표시하는지 확인한다.

**인수 시나리오**:

1. **Given** 사용자가 대화를 시작하고 "InBody 770S" 또는 "770S"를 입력한 상태, **When** 시스템이 입력을 분석하면, **Then** 770S 전문가용 모드로 분기하고, 전문적 톤앤매너로 환영 메시지를 표시한다.
2. **Given** 사용자가 기종을 명시하지 않고 "체성분 분석기 문제가 있어요"라고 입력한 상태, **When** 시스템이 기종 정보가 없음을 감지하면, **Then** 4개 기종 선택지(270S, 580, 770S, 970S)를 표시한다.
3. **Given** 사용자가 지원하지 않는 기종(예: "InBody 120")을 입력한 상태, **When** 시스템이 지원 범위 밖임을 감지하면, **Then** 지원 불가 안내와 함께 InBody 공식 고객센터 연락처를 제공한다.
...
### User Story 6 - 채팅 UI 및 배포 (Priority: P6)

사용자가 웹 브라우저를 통해 InBody 기술 지원 챗봇에 접속하면, Streamlit 기반 채팅 인터페이스에서 대화를 진행할 수 있다. 기종 선택, 스트리밍 응답 등의 기능을 GUI로 제공하며, AWS EC2에 배포되어 평일 09:00~19:00에 자동으로 운영된다.

관리자는 별도의 관리 페이지에서 기종별 PDF 문서(매뉴얼, 프린터 호환리스트, 측정시 주의사항)를 업로드하고 RAG 인제스트를 수행할 수 있다.

**우선순위 근거**: 모든 백엔드 에이전트가 구현된 후에 프론트엔드를 연결해야 완전한 사용자 경험을 제공할 수 있다. 배포는 데모 시연을 위한 최종 단계이다.

**독립 테스트**: 브라우저에서 Streamlit UI에 접속하여 기종을 선택하고, 기술 지원 질문을 입력했을 때 스트리밍으로 응답이 표시되는지 확인한다. 관리 페이지에서 PDF 업로드 후 해당 문서 기반 응답이 생성되는지 확인한다.

**인수 시나리오**:

1. **Given** 사용자가 브라우저에서 Streamlit UI에 접속한 상태, **When** 사이드바에서 "InBody 770S"를 선택하고 채팅 입력창에 질문을 입력하면, **Then** 전문가용 톤으로 스트리밍 응답이 실시간 표시된다.
2. **Given** 사용자가 기종을 선택하지 않은 상태, **When** 채팅 입력창에 질문을 입력하면, **Then** 기종 확인 질문이 표시되고 사이드바에서 기종 선택을 유도한다.
3. **Given** EC2 인스턴스가 스케줄에 따라 09:00 KST에 시작된 상태, **When** 사용자가 브라우저로 접속하면, **Then** Streamlit UI와 FastAPI 백엔드가 정상 동작한다.
4. **Given** 관리자가 관리 페이지에 접속한 상태, **When** 기종(예: 770S)을 선택하고 매뉴얼 PDF를 업로드하면, **Then** 해당 PDF가 기종별 벡터 DB 컬렉션에 인제스트되고 완료 메시지를 표시한다.
5. **Given** 관리자가 770S의 프린터 호환리스트 PDF를 업로드한 상태, **When** 일반 사용자가 "770S 프린터 연결" 질문을 하면, **Then** 업로드된 호환리스트 기반으로 응답이 생성된다.
6. **Given** 관리자가 측정시 주의사항 PDF를 공통 문서로 업로드한 상태, **When** 어떤 기종의 사용자라도 측정 관련 질문을 하면, **Then** 해당 공통 문서 내용이 응답에 참조된다.
7. **Given** 사용자가 처음 UI에 접속하여 아직 메시지를 보내지 않은 상태, **When** 웰컴 화면이 표시되면, **Then** 카테고리별(설치/연동/트러블슈팅/측정결과) 예시 질문 버튼이 표시되고, 클릭 시 해당 질문이 자동으로 입력된다.
8. **Given** 사용자가 질문을 입력하고 에이전트가 응답을 생성하는 중인 상태, **When** SSE 스트리밍으로 토큰이 전달되면, **Then** 라우터/가드레일 등 내부 처리 노드의 JSON 출력은 사용자에게 노출되지 않고, 에이전트의 자연어 응답만 실시간 표시된다.

---

### User Story 7 - 시멘틱 캐싱 (Priority: P7)

현재 시스템은 모든 사용자 질문에 대해 LLM을 2~8회 호출(ModelRouter → IntentRouter → Agent → Guardrail)한다. 기술 지원 챗봇의 특성상 동일하거나 유사한 질문이 반복되므로, 이전에 생성된 응답을 의미적 유사도 기반으로 캐싱하여 LLM 호출 비용과 응답 지연 시간을 절감한다.

**우선순위 근거**: 기능적으로는 기존 시스템이 완전히 동작하는 상태이므로, 비용 최적화와 성능 향상을 위한 후순위 기능이다. 단, 운영 비용에 직접적 영향을 미치므로 배포 후 조기에 적용하는 것이 효과적이다.

**독립 테스트**: 동일한 기종+질문 조합을 두 번 전송했을 때, 두 번째 요청에서 LLM을 호출하지 않고 캐시된 응답을 반환하며, 응답 내용이 동일한지 확인한다.

**인수 시나리오**:

1. **Given** 770S 사용자가 "프린터 연결 방법"을 질문하여 정상 응답을 받은 상태, **When** 다른 세션의 770S 사용자가 "프린터를 어떻게 연결하나요?"라고 유사한 질문을 하면, **Then** LLM 호출 없이 캐시된 응답을 반환하고, 응답에 캐시 히트 여부를 메타데이터로 포함한다.
2. **Given** 270S 사용자가 "에러 코드 E001"을 질문하여 캐시된 상태, **When** 580 사용자가 동일한 "에러 코드 E001"을 질문하면, **Then** 기종이 다르므로 캐시를 사용하지 않고 580 전용 워크플로우를 실행한다.
3. **Given** 캐시에 30일 이상 경과한 응답이 존재하는 상태, **When** TTL이 만료되면, **Then** 해당 캐시 엔트리가 자동으로 무효화되고, 다음 동일 질문 시 LLM을 다시 호출한다.
4. **Given** 관리자가 에러 코드 DB를 업데이트한 상태, **When** 캐시 무효화 API를 호출하면, **Then** 해당 기종+의도 조합의 캐시가 전체 삭제된다.
...
- **FR-001**: 시스템은 반드시 사용자의 텍스트 입력 또는 기종 선택을 통해 InBody 기종(270S, 580, 770S, 970S)을 식별할 수 있어야 한다.
- **FR-002**: ~~시스템은 반드시 사용자가 업로드한 기기 이미지에서 InBody 기종을 식별할 수 있어야 한다.~~ **(제거 — 데모에서 오버엔지니어링으로 판단)**
...
- **FR-015**: 시스템은 Streamlit 기반 웹 채팅 UI를 제공하여, 사용자가 브라우저에서 대화형으로 기술 지원을 받을 수 있어야 한다.
- **FR-016**: 채팅 UI는 SSE 스트리밍을 통해 응답을 실시간으로 표시해야 한다.
- **FR-017**: 시스템은 Docker Compose로 패키징되어 AWS EC2에 배포 가능해야 한다.
- **FR-018**: EC2 인스턴스는 EventBridge 스케줄을 통해 평일 09:00~19:00 KST에만 자동 운영되어야 한다.
- **FR-019**: 관리자 전용 페이지에서 기종을 선택한 후, PDF 문서를 카테고리별(매뉴얼, 프린터 호환리스트, 측정시 주의사항)로 업로드할 수 있어야 한다.
- **FR-020**: 매뉴얼 PDF는 기종당 필수 업로드이며, 프린터 호환리스트와 측정시 주의사항은 선택 업로드이다.
- **FR-021**: 측정시 주의사항 PDF는 전 기종 공통 문서로, 한 번 업로드하면 모든 기종의 RAG 검색에서 참조되어야 한다.
- **FR-022**: PDF 업로드 시 자동으로 기종별 벡터 DB 컬렉션에 인제스트(청킹 + 임베딩 + 메타데이터 태깅)되어야 한다.
- **FR-023**: 트러블슈팅 에이전트는 에러 코드 DB에서 해당 기종의 에러 코드를 찾을 수 없는 경우 절대 추측하여 응답을 생성하지 않아야 하며, '해당 기종에 등록되지 않은 에러 코드'임을 안내하고 서비스 센터 문의를 권고해야 한다.
- **FR-024**: 모든 전문 에이전트는 기종별 매뉴얼 목차 구조(섹션 맵)를 활용하여, 현재 응답과 관련된 인접 섹션 정보를 기반으로 맥락 있는 후속 질문을 1~2개 제안해야 한다.
- **FR-025**: 채팅 UI는 대화가 시작되지 않은 상태에서 카테고리별(설치/연동/트러블슈팅/측정결과) 예시 질문을 버튼 형태로 표시하여, 사용자가 시스템의 지원 범위를 직관적으로 파악할 수 있어야 한다.
- **FR-026**: SSE 스트리밍 시 내부 처리 노드(라우터, 가드레일 등)의 LLM 토큰은 사용자 UI에 노출되지 않아야 하며, 전문 에이전트 노드의 응답 토큰만 스트리밍되어야 한다.
- **FR-027**: 시스템은 사용자 질문의 의미적 유사도를 기반으로 이전 응답을 캐싱하여, 유사도 임계값(기본 0.92) 이상의 질문에 대해 LLM 호출 없이 캐시된 응답을 반환해야 한다.
- **FR-028**: 시멘틱 캐시는 반드시 기종(model_id)별로 격리되어야 하며, 다른 기종의 캐시된 응답이 반환되어서는 안 된다.
- **FR-029**: 시멘틱 캐시는 의도(intent)별로 서로 다른 TTL을 적용해야 한다 (troubleshoot: 7일, install: 30일, connect: 14일, clinical: 90일).
- **FR-030**: guardrail_passed=True인 응답만 캐시에 저장되어야 하며, 가드레일을 통과하지 못한 응답은 캐시하지 않아야 한다.
- **FR-031**: 관리자가 API를 통해 특정 기종 또는 특정 기종+의도 조합의 캐시를 수동으로 무효화할 수 있어야 한다.
- **FR-032**: SSE 스트리밍 엔드포인트에서 캐시 히트 시, 전체 응답을 단일 이벤트로 즉시 반환하고 done 이벤트의 메타데이터에 cache_hit=true를 포함해야 한다.
...
- **SC-002**: ~~시스템의 이미지 기반 기종 식별 정확도가 85% 이상이어야 한다.~~ **(제거 — FR-002와 함께 제거)**
...
- **SC-009**: 에러 코드 DB에 등록되지 않은 에러 코드에 대해 추측 응답을 생성하는 비율이 0%이어야 한다.
- **SC-010**: 시멘틱 캐시의 동일 기종+유사 질문에 대한 히트율이 60% 이상이어야 한다.
- **SC-011**: 캐시 히트 시 응답 지연 시간이 200ms 이하이어야 한다 (LLM 미호출).
- **SC-012**: 캐시된 응답이 다른 기종의 정보를 포함하는 교차 오염 발생률이 0%이어야 한다.
