...
edges.py의 3개 라우팅 함수가 상태에 따라 올바른 노드를 반환하는지 검증한다.
"""
...
@pytest.mark.unit
def test_model_router_identified_goes_to_intent():
    """identified_model이 설정되면 intent_router로 라우팅한다."""
    assert route_after_model_router({"identified_model": "270S"}) == "intent_router"
...
@pytest.mark.unit
def test_guardrail_passed_ends():
    """guardrail_passed=True이면 __end__로 라우팅한다."""
    assert route_after_guardrail({"guardrail_passed": True}) == "__end__"
...
@pytest.mark.unit
def test_guardrail_max_retry_ends():
    """retry_count >= MAX이면 __end__로 라우팅한다."""
    state = {"guardrail_passed": False, "guardrail_retry_count": MAX_GUARDRAIL_RETRIES}
    assert route_after_guardrail(state) == "__end__"
